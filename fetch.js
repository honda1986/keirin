...
const WAIT_MS = 1000;           // 連続アクセスの間隔(サーバー負荷への配慮)
const MAX_RACES = 150;          // 安全上限
...
async function main() {
  ...
  for (const url of queue) {
    ...
    try {
      ...
      if (!/基本出走データ/.test(html)) {
        collectLinks(html, url).forEach((u) => { if (!seen.has(u) && queue.length < 600) { seen.add(u); queue.push(u); } });
        continue;
      }
      ...
      console.log("OK:", p.place, p.raceNo, sx ? sx.score + "%" : "girls");
    } catch (e) {
  ...
  const outPath = path.join(__dirname, "..", "docs", "races.json");
