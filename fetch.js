...
async function main() {
  ...
  for (const url of queue) {
    ...
    try {
      ...
      // === 診断: 最初の1ページだけ、並び予想周辺の生HTMLとtitleをログに出す ===
      if (!global.__dumped) {
        global.__dumped = true;
        const ti = html.match(/<title>[^<]*<\/title>/);
        console.log("DEBUG title:", ti ? ti[0] : "(no title)");
        const ni = html.indexOf("並び予想");
        if (ni !== -1) {
          console.log("DEBUG 並びHTML>>>");
          console.log(html.slice(ni, ni + 1200).replace(/\n/g, "\\n"));
          console.log("<<<DEBUG end");
        } else {
          console.log("DEBUG: ページ内に「並び予想」の文字なし");
        }
      }
