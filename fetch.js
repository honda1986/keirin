...
// レースページ1件を取得して races へ格納(成功したら true)
...
async function main() {
  ...
  // ---- 第1段: pid直接列挙(URL規則: /keirin/yoso/?rdt=YYYY-MM-DD&pid=N) ----
  const jst = new Date(Date.now() + 9 * 3600 * 1000);
  const rdt = jst.toISOString().slice(0, 10);
  console.log("phase1: pid列挙", rdt);
  let miss = 0;
  for (let pid = 1; pid <= 160; pid++) {
    ...
    try {
      await sleep(WAIT_MS);
      const ok = await fetchRace(url, races);
      miss = ok ? 0 : miss + 1;
      if (miss >= 25) { console.log("pid連続空振り25回で列挙終了 (pid=" + pid + ")"); break; }
    } catch (e) { miss++; if (miss >= 25) break; }
  }
