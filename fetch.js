...
function processRace(html, url, races) {
  ...
  races.push({
    key, place: p.place, raceNo: p.raceNo, startTime: p.startTime || "", grade: p.grade, date: p.date,
    ...
    riders: (() => { const posOf = {}; for (const l of p.lines) { if (l.length === 1) posOf[l[0]] = 3; else l.forEach((c, li) => { posOf[c] = Math.min(li, 2); }); } const rankOf = {}; r.scores.forEach((sc, ri) => (rankOf[sc.car] = ri + 1)); return p.entries.map((en) => [en.car, en.age || 0, parseInt(en.ki) || 0, posOf[en.car] ?? 3, rankOf[en.car] || 9, +(r.scores.find((sc) => sc.car === en.car)?.total || 0).toFixed(1)]); })(),
