...
echo.
echo   試しに1回動かしてみます(30秒ほど)...
schtasks /run /tn keirin_snap >nul 2>&1
timeout /t 30 /nobreak >nul

rem 日付の見た目は地域設定で変わるので、date /t は当てにしない
for /f %%d in ('powershell -NoProfile -Command "Get-Date -Format yyyyMMdd"') do set TODAY=%%d
set "LOG=%~dp0..\..\logs\snap_%TODAY%.log"
echo.
if exist "%LOG%" (
  echo   --- %LOG% の終わりのほう ---
  ...
  echo   ログがまだありません: %LOG%
  echo   少し待ってから、このフォルダを見てください。
)

echo.
echo   ここまで来たら、あとは毎日ひとりでに動きます(07:50 から翌 01:00 まで1分おき)。
echo.
pause
