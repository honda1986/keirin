...
if (LW) {
  for (const e of ALL) {
    ...
    e.adjRanks = adj; adjCount++;
  }
  console.log("補正後評価順を再構築:", adjCount, "レース (weights " + LW.updatedAt + ")");
}
...
function evaluate(patternFn, filterFn, useAdj) {
  ...
  return {
    ...
    profit: Math.round(ret - cost), breakEvenOdds: hits ? +((cost / hits) / 100).toFixed(1) : null, // 収支+損益分岐オッズ
  };
}
...
const winners = results
  .filter((r) => r.roi >= 100 && (adjCount >= 100 ? r.adj : !r.adj) && PATTERN_ID[stripAdj(r.pattern)] && FILTER_COND[r.filter])
  .map((r) => ({
    roi: r.roi, hitRate: r.hitRate, avgPoints: r.avgPoints, races: r.races,
    patternId: PATTERN_ID[stripAdj(r.pattern)], patternName: stripAdj(r.pattern),
...
const allPatterns = results
  .filter((r) => (adjCount >= 100 ? r.adj : !r.adj) && PATTERN_ID[stripAdj(r.pattern)] && FILTER_COND[r.filter] && r.races >= 30)
  .map((r) => ({
    roi: r.roi, hitRate: r.hitRate, avgPoints: r.avgPoints, races: r.races,
    patternId: PATTERN_ID[stripAdj(r.pattern)], patternName: stripAdj(r.pattern),
    filterName: r.filter, cond: FILTER_COND[r.filter],
