...
function parseHaraiList(html) {
  ...
  while ((m = tokenRe.exec(html))) {
    ...
    // 【重要】賭式は区切り文字で判別する。数字の個数だけで判別すると
    //   3連複(3=5=7)を3連単、2車複(3=7)を2車単として誤って取り込む。
    //   順序あり(3連単・2車単)は "-"、順序なし(3連複・2車複・ワイド)は "="。
    //   複式は着順ではなく番号順に並ぶため、取り込むと着順データまで壊れる。
    const syms = [...orderChunk.matchAll(/class="symbol"[^>]*>([\s\S]{0,8}?)<\//g)].map((x) => x[1]);
    if (syms.some((s) => s.includes("=") || s.includes("＝"))) continue; // 複式は無視
