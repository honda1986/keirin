...
async function main() {
  ...
  for (const dH of dates) {
    ...
    try {
      ...
      const md0 = toMd(hhtml);
      if (!global.__h2) { global.__h2 = true;
        const i3 = hhtml.indexOf("3連単");
        console.log("DBG 3連単周辺>>>", i3 >= 0 ? hhtml.slice(i3 - 60, i3 + 200).replace(/\s+/g, " ") : "(3連単の文字なし)");
        const i2 = hhtml.indexOf("2車単");
        console.log("DBG 2車単周辺>>>", i2 >= 0 ? hhtml.slice(i2 - 20, i2 + 200).replace(/\s+/g, " ") : "(2車単なし)");
        const ir = hhtml.search(/1R|１Ｒ/);
        console.log("DBG 1R周辺>>>", ir >= 0 ? hhtml.slice(ir - 30, ir + 250).replace(/\s+/g, " ") : "(1Rなし)");
        console.log("DBG toMd後の3連単周辺>>>", (()=>{const j=md0.indexOf("3連単");return j>=0?md0.slice(j,j+250).replace(/\s+/g," "):"(md変換後に3連単なし)";})());
      }
      results = parseHaraiList(md0);
      if (!global.__hdbg) {
        global.__hdbg = true;
