...
function parseHaraiList(html) {
  const out = {};
  // 【重要】まず「○○競輪」見出しでブロック分割してから各ブロック内の行を読む。
  //   1つの正規表現で通し読みすると、配当が空の行(未発売等)で次の配当を探して
  //   1000字先まで走り、その途中の場名見出しを飲み込んで別の場のレースを
  //   前の場に誤記録してしまう(3連単だけ欠ける等の原因)。
  ...
  for (let i = 1; i < parts.length; i += 2) {
    ...
    while ((m = rowRe.exec(block))) {
      // 1件のマッチが複数行にまたがっていたら誤読なので捨てる(配当が空の行を飛び越えたケース)
      if ((m[0].match(/class="race"/g) || []).length > 1) continue;
      const rno = m[1], orderChunk = m[2], pay = +m[3].replace(/,/g, "");
...
async function main() {
  ...
  for (const dH of dates) {
    ...
    if (dayEntries.length >= 20) {
      ...
      console.log(dH, "収集済み(" + dayEntries.length + "R) → 出走表取得スキップ / 2車単追記", dp2, "件");
      continue;
    }
