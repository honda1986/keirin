...
function parseHaraiList(html) {
  ...
  for (let i = 1; i < parts.length; i += 2) {
    ...
    for (let k = 0; k < heads.length; k++) {
      ...
      if (!payM) continue;                       // 配当なし(未発売・未確定)はスキップ
...
function predictOne(html, url) {
  ...
  return {
    place: p.place, raceNo: p.raceNo, klass: r.klass, grade: p.grade || "", pattern: r.linePattern,
...
async function main() {
  ...
  const entryById = new Map(hist.entries.map((e) => [e.id, e])); // 既存エントリへの2車単追記用
  ...
  for (const dH of dates) {
    ...
    // 1.5) この日が既に十分収集済みなら、Gamboo出走表の取得(遅い)を丸ごとスキップして
    //      既存エントリへの2車単配当(p2pay)追記だけ行う → days=30 でも数分で完了
    const d8pre = dH.replace(/-/g, "");
    const dayEntries = hist.entries.filter((e) => e.date === d8pre);
    // 【重要】固定値(20R)で判定すると、中途半端にしか集まっていない日が
    //   永久に補完されない。払戻ページで確定しているレース数と比べて、
    //   9割以上そろっている日だけスキップする。
    const confirmed = Object.values(results).filter((r) => r.first != null || r.p2pay != null).length;
    const enough = confirmed > 0 ? dayEntries.length >= confirmed * 0.9 : dayEntries.length >= 20;
    if (enough) {
      let dp2 = 0;
      for (const ex of dayEntries) {
        if (ex.p2pay != null) continue;
        const res = results[ex.place + "_" + ex.raceNo];
        if (res && res.p2pay != null) { ex.p2pay = res.p2pay; p2added++; dp2++; }
      }
      console.log(dH, "収集済み(" + dayEntries.length + "/" + confirmed + "R) → 出走表取得スキップ / 2車単追記", dp2, "件");
      continue;
    }

    if (dayEntries.length) console.log(dH, "不足(" + dayEntries.length + "/" + confirmed + "R) → 出走表を取得して補完します");
    ...
    await pool(urls, async (item) => {
      ...
      // 3連単が取れなくても、2車単の並びが「1着-2着」なので着順を補完できる
      if (res && res.first == null && res.p2first != null) {
        res.first = res.p2first; res.second = res.p2second; res.third = null; res.p3pay = null;
      }
      if (!res) {
      ...
      if (done.has(eid)) {
        // 既存レース: 2車単配当(p2pay)が未設定なら後から追記する(過去データの2車単補完)
        if (res.p2pay != null) {
          const ex = entryById.get(eid);
          if (ex && ex.p2pay == null) { ex.p2pay = res.p2pay; p2added++; }
        }
        return;
      }
      const { first: f, second: s, third: t, p3pay } = res;
      const newEntry = {
        id: eid, date: dH.replace(/-/g, ""), place: pr.place, raceNo: pr.raceNo, klass: pr.klass, grade: pr.grade || "",
      ...
      entryById.set(eid, newEntry);
      done.add(eid); added++; dayAdded++;
    });
    saveNow(); // 途中で強制終了されても、ここまでの成果は残る
