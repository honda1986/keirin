...
function parseHaraiList(html) {
  ...
  const tokenRe = /([぀-ヿ一-龥]{2,5})競輪|class="race"[^>]*>\s*(\d{1,2})R[\s\S]*?class="order"([\s\S]*?)<\/td>[\s\S]*?class="refund"[^>]*>\s*([\d,]+)/g;
  let m, venue = null;
...
async function main() {
  ...
  const stratEval = (e) => {
    ...
    for (const w of PATS) {
      ...
      const useAdj = w.adj === true || /補正後/.test(w.patternName || "");
      let rk = e.ranks;
      if (useAdj) {
        if (!adjCache.has(e.id)) adjCache.set(e.id, adjRanksOf(e));
        const ar = adjCache.get(e.id);
        if (!ar) continue; // 補正順が作れないレースは[補正後]パターンの対象外
        rk = ar;
      }
      const t = b(rk, e.lines);
  ...
  const topEval = (e) => {
    ...
    const useAdj = TOP.adj === true || /補正後/.test(TOP.patternName || "");
    let rk = e.ranks;
    if (useAdj) {
      if (!adjCache.has(e.id)) adjCache.set(e.id, adjRanksOf(e));
      const ar = adjCache.get(e.id);
