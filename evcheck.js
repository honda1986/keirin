...
const app = new Function(html.slice(a, b) + "\nreturn { EV_MODEL, VENUE_PREF, evDelta, evOfRow };")();
...
for (let t = 0; t < 1000; t++) {
  ...
  const p = { place, lines, entries: ents.map((e) => ({ car: e.car, age: e.age, ki: e.ki + "期", pref: e.pref })) };
  const r = { scores: order.map((car) => ({ car })) };
  const ea = app.evOfRow({ delta: app.evDelta(p, r), ticket, cars }, { o, n: cars });
  // 夜の集計: history の riders 形式から
  const riders = ents.map((e) => [e.car, e.age, e.ki, 0, order.indexOf(e.car) + 1, 0, 0, e.pref]);
