...
// ===== 月別のデータ充足状況(どこが欠けているかを可視化) =====
{
  const m = {};
  for (const e of E) {
    if (!e.date) continue;
    const k = e.date.slice(0, 6);
    const v = (m[k] = m[k] || { all: 0, p2: 0, ranks: 0, lines: 0, usable: 0 });
    v.all++;
    if (e.p2pay != null) v.p2++;
    if (Array.isArray(e.ranks) && e.ranks.length >= 4) v.ranks++;
    if (Array.isArray(e.lines) && e.lines.length) v.lines++;
    if (e.p2pay != null && Array.isArray(e.ranks) && e.ranks.length >= 4 && Array.isArray(e.lines) && e.lines.length) v.usable++;
  }
  console.log("\n=== 月別のデータ充足状況 ===");
  console.log("月        全件   2車単配当   評価順   ライン   分析可能   欠落率");
  for (const k of Object.keys(m).sort()) {
    const v = m[k];
    const lost = v.all ? ((v.all - v.usable) / v.all * 100) : 0;
    console.log("  " + k.slice(0, 4) + "/" + k.slice(4) + "  " + String(v.all).padStart(5) + "  " +
      String(v.p2).padStart(8) + "  " + String(v.ranks).padStart(7) + "  " + String(v.lines).padStart(7) + "  " +
      String(v.usable).padStart(8) + "  " + lost.toFixed(1).padStart(5) + "%" + (lost > 20 ? "  ⚠" : ""));
  }
  console.log("  ※ 欠落率が高い月は、配当・評価順・ラインのどれかが取れていません。");
  console.log("    2車単配当が少ない → 払戻ページの読み取り失敗の可能性。");
  console.log("    評価順・ラインが少ない → 出走表の取得失敗(過去レース収集で補完できます)。");
}
