...
function parseWinticket(text, trackNames) {
  ...
  const mST = joined.match(/発走(?:予定)?\s*(\d{1,2}):(\d{2})/) || joined.match(/(\d{1,2}):(\d{2})\s*発走/);
  meta.startTime = mST ? mST[1].padStart(2, "0") + ":" + mST[2] : "";
...
function parseGamboo(text, trackNames) {
  ...
  if (!meta.startTime) {
    const mt = text.match(/発走(?:予定)?\s*(\d{1,2}):(\d{2})/) || text.match(/(\d{1,2}):(\d{2})\s*発走/);
    if (mt) meta.startTime = mt[1].padStart(2, "0") + ":" + mt[2];
  }
  if (!meta.date) {
...
// ---- kdreams(楽天Kドリームス)のレース詳細ページ用パーサー ----
// 目印は「府県/年齢/期別」の行(例: 静　岡/24/121)。そこから前後を読む。
//   -2行: 車番   -1行: 選手名
//   +1: 級班  +2: 脚質  +3: ギヤ  +4: 競走得点
//   +5..+10: S B 逃 捲 差 マ
//   +11..+14: 1着 2着 3着 着外
//   +15..+17: 勝率 2連対率 3連対率
// 並びは「並び予想」以降の「車番 + 先行/追込/押え先…」の列から復元する。
//   先行・押え先・自力系 = 新しいラインの先頭 / 追込 = 直前のラインに続く
function parseKdreams(text, trackNames) {
  const L = String(text).split(/\r?\n/).map((s) => s.trim()).filter(Boolean);
  const joined = L.join("\n");

  const meta = {};
  { let best = "", bi = Infinity;
    for (const n of trackNames || []) { const i = joined.indexOf(n); if (i !== -1 && i < bi) { bi = i; best = n; } }
    meta.place = best; }
  const mR = joined.match(/(\d{1,2})\s*R/);           meta.raceNo = mR ? mR[1] + "R" : "";
  const mG = joined.match(/[ＳＡＬSAL]級[^\n]{0,8}/);  meta.grade = mG ? mG[0].trim() : "";
  const mD = joined.match(/(\d{4})年(\d{1,2})月(\d{1,2})日/); meta.date = mD ? mD[0] : "";
  const iSt = L.findIndex((x) => /^発走予定/.test(x));
  const mSt = iSt >= 0 ? (L[iSt].match(/(\d{1,2}):(\d{2})/) || (L[iSt + 1] || "").match(/^(\d{1,2}):(\d{2})$/)) : null;
  meta.startTime = mSt ? mSt[1].padStart(2, "0") + ":" + mSt[2] : "";
  meta.weather = ""; meta.windSpeed = null;   // kdreamsには無い(Gamboo版も同じ)

  const PROF = /^([^\/\s]{1,6}[\s　]?[^\/\s]{0,6})\/(\d{1,2})\/(\d{1,3})$/;
  const isNum = (s) => /^-?\d{1,3}(\.\d{1,2})?$/.test(s);
  const numOf = (s) => { const v = parseFloat(s); return isNaN(v) ? 0 : v; };

  const entries = []; const seen = new Set();
  for (let i = 0; i < L.length; i++) {
    const pm = L[i].match(PROF);
    if (!pm) continue;
  ...
  if (entries.length < 5) throw new Error("kdreams: 選手データを" + entries.length + "名しか読み取れませんでした");
...
// Gamboo / WINTICKET にはこの形が無いので、これで振り分けられる。
function looksKdreams(text) {
  if (/基本出走データ/.test(text)) return false;          // Gamboo は対象外
  if (!/並び予想/.test(text)) return false;                // kdreams には必ずある
  const re = /^[^\/\s]{1,6}[\s　]?[^\/\s]{0,6}\/\d{1,2}\/\d{1,3}$/;
  let n = 0;
  for (const l of String(text).split(/\r?\n/)) { if (re.test(l.trim())) n++; if (n >= 5) return true; }
  return false;
}

function parseCard(text, trackNames) {
  // 判定の順序が大事: Gamboo → kdreams → WINTICKET。
  // Gamboo の生データにも「県/数字/数字」に見える行があるので、Gamboo を先に確かめる。
  ...
  if (looksKdreams(text)) {
    try { return parseKdreams(text, trackNames); } catch (e) { /* fallback */ }
  }
...
function predict(parsed, bank, trackName, learnW) {
  ...
  const scores = es.map((e) => {
    ...
    if (learnW) {
      ...
      // 位置×年齢の交互作用(learn.js が「3着以内率」で学習した残差)。
      // 位置と年齢を別々に足すだけでは表せない組み合わせがある。実測の乖離(3着以内):
      //   単騎 28-35歳 +1.8pt / 単騎 50歳〜 -11.6pt   ← 単騎は年齢で真逆
      //   先頭 〜23歳   +4.2pt / 先頭 44歳〜   -8.1pt   ← 高齢の先行役は先行しきれない
      const a6 = a > 0 ? (a <= 23 ? "〜23" : a <= 27 ? "24-27" : a <= 35 ? "28-35" : a <= 43 ? "36-43" : a <= 49 ? "44-49" : "50〜") : null;
      if (a6 && learnW.posAgeBonus && learnW.posAgeBonus[posKey + "|" + a6]) lw += learnW.posAgeBonus[posKey + "|" + a6];
    }
