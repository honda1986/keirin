...
function parseGamboo(text, trackNames) {
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    let j = i + 1;
    const nums = [];
    // 級班・脚質・得点(現/前)・失違・決まり手4・B・率4 を順に収集
    while (j < tokens.length && !tokens[j].match(profRe) && tokens[j] !== "並び予想") {
      ...
      if (e.score && nums.length < 10 && (isInt(t) || isFloat(t))) {
        nums.push(parseFloat(t)); j++; continue;
      }
      if (e.score && nums.length >= 10) break; // 失違+決まり手4+B+率4の10個で数値列は終わり→成績欄へ
      j++;
    }
    // nums: [失違, 逃, 捲, 差, マ, B, (以降 4値)]
    if (nums.length >= 10) {
      e.k = { nige: nums[1], makuri: nums[2], sashi: nums[3], mark: nums[4] };
      e.B = nums[5]; e.H = nums[5]; // Gamboo出走データにH数は無いためBで近似
      const w1 = e.k.nige + e.k.makuri + e.k.sashi + e.k.mark; // 決まり手合計=1着回数
      if (countMode) {
        // [1着回数, 2着回数, 3着回数, 着外回数]
        const c1 = nums[6], c2 = nums[7], c3 = nums[8], cOut = nums[9];
        ...
        // [勝率, 2連対率, 3連対率, 着外率]
        e.rate = { win: nums[6], niren: nums[7], sanren: nums[8] };
        const outRate = nums[9];
...
function predict(parsed, bank, trackName) {
  ...
  const scores = es.map((e) => {
    ...
    const total = ptScore + form + wins + fit + pos + sB + gap;
    return { car: e.car, name: e.name, total, br: { 得点: ptScore, 調子: form, 勝ち星: wins, 適性: fit, 位置: pos, S: sB, 乖離: gap }, role: r };
  ...
  if (klass === "girls" || !mainLine) {
    // ライン概念なし: 評価順で素直に
    first = uniq([a1, scores[1] && scores[0].total - scores[1].total < 3 ? a2 : null]);
    second = uniq([a1, a2, a3, a4]);
    third = uniq([a2, a3, a4, a5x]);
  } else {
    const mh = mainLine[0], ms = mainLine[1] != null ? mainLine[1] : null;
    const rivals = scores.map((sc) => sc.car)
      .filter((c) => !mainLine.includes(c) && c !== strongTanki && (role[c] !== "単騎" || inTankis.includes(c)))
      .slice(0, 3);
    if (strongTanki != null) {
      first = [strongTanki];
      second = uniq([mh, ms, rivals[0]]);
      third = uniq([mh, ms, rivals[0], rivals[1]]);
    } else {
      // 本線ライン内で評価上位の選手を1着に(先頭固定にしない)。先頭と番手の評価差が小さければ両方1着へ
      const inHead = mainLine.filter((c) => c != null).sort((x, y) => (totalOf[y] || 0) - (totalOf[x] || 0));
      first = [inHead[0]];
      if (inHead[1] != null && (totalOf[inHead[0]] || 0) - (totalOf[inHead[1]] || 0) < 6) first.push(inHead[1]);
      second = uniq([ms, mh, rivals[0], rivals[1], ...inTankis.slice(0, 1)]);
      third = uniq([ms, mh, mainLine[2], rivals[0], rivals[1], rivals[2], ...inTankis]).slice(0, 6);
    }
  }
  const nishatan = [], sanrentan = [];
  const addN = (x, y) => { if (x == null || y == null || x === y) return; const t = x + "-" + y; if (!nishatan.includes(t)) nishatan.push(t); };
  const addS = (x, y, z) => { if (x == null || y == null || z == null || x === y || y === z || x === z) return; const t = x + "-" + y + "-" + z; if (!sanrentan.includes(t)) sanrentan.push(t); };
  first.forEach((f) => second.forEach((s2c) => addN(f, s2c)));
  // 妙味スジ(回収率90%以上)は2車単に追加(研究所020)
  sujiPlan.filter((sp) => sp.roi >= 90).forEach((sp) => { const [x, y] = sp.ticket.split("-").map(Number); addN(x, y); });
  first.forEach((f) => second.forEach((s2c) => third.forEach((t3) => addS(f, s2c, t3))));
  // 第2ライン妙味スジの絡み(研究所020)
  const s2 = sujiPlan.find((sp) => sp.rank === 2 && sp.roi >= 85);
  if (s2 && s2.line[1] != null) { addS(s2.line[0], s2.line[1], first[0]); addS(first[0], s2.line[0], s2.line[1]); }
